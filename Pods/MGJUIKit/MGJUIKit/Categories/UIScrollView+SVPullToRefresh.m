...
static void (^customViewConfigureBlock)(UIScrollView *);
...
@property (nonatomic, strong) NSMutableArray *addtionalActionForState;
...
+ (void)configureCustomViewWithBlock:(void (^)(UIScrollView *))block
{
    customViewConfigureBlock = block;
}
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler {
    if(!self.pullToRefreshView) {
        ...
        if (customViewConfigureBlock) {
            customViewConfigureBlock(self);
        }
    }
}
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"Pull to refresh...",),
                                                       NSLocalizedString(@"Going to refresh...",),
        ...
        self.addtionalActionForState = [NSMutableArray arrayWithObjects:@"", @"", @"", @"", @"", nil];
    }
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        if (![[self.addtionalActionForState objectAtIndex:self.state] isKindOfClass:[NSString class]]) {
            ((void (^)())[self.addtionalActionForState objectAtIndex:self.state])();
        }
    }
    else {
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        else if(contentOffset.y < scrollOffsetThreshold && self.scrollView.isDragging && self.state != SVPullToRefreshStateTriggered)
            self.state = SVPullToRefreshStateTriggered;
        else if (contentOffset.y < -self.originalTopInset && contentOffset.y > scrollOffsetThreshold && self.state != SVPullToRefreshStateTriggering)
            self.state = SVPullToRefreshStateTriggering;
        
        if(contentOffset.y >= -self.originalTopInset && self.state != SVPullToRefreshStateStopped)
            self.state = SVPullToRefreshStateStopped;
    } else {
...
- (void)setCustomViewAddtionalActionBlock:(void (^)())block forState:(SVPullToRefreshState)state
{
    if (state == SVPullToRefreshStateAll) {
        [self.addtionalActionForState replaceObjectsInRange:NSMakeRange(0, 4) withObjectsFromArray:@[block, block,block, block]];
    } else {
        [self.addtionalActionForState replaceObjectAtIndex:state withObject:block];
    }
}
