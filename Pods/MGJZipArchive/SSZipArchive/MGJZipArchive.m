...
@interface MGJZipArchive ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end


@implementation MGJZipArchive {
	NSString *_path;
	NSString *_filename;
    zipFile _zip;
}
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<SSZipArchiveDelegate>)delegate {
	...
	do {
		@autoreleasepool {
	        ...
	        if (nil != err) {
	            NSLog(@"[MGJZipArchive] Error: %@", err.localizedDescription);
	        }
	            ...
	            if (fp) {
	                ...
	                if (fileInfo.dosDate != 0) {
	                    ...
	                    if (attr) {
	                        if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
	                            ...
	                            NSLog(@"[MGJZipArchive] Failed to set attributes - whilst setting modification date");
	                        }
	                    }
	                }
                    ...
                    if (permissions != 0) {
                        ...
                        if ([fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil] == NO) {
                            ...
                            NSLog(@"[MGJZipArchive] Failed to set attributes - whilst setting permissions");
                        }
    ...
    for (NSDictionary * d in directoriesModificationDates) {
        if (![[NSFileManager defaultManager] setAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[d objectForKey:@"modDate"], NSFileModificationDate, nil] ofItemAtPath:[d objectForKey:@"path"] error:&err]) {
            NSLog(@"[MGJZipArchive] Set attributes failed for directory: %@.", [d objectForKey:@"path"]);
        }
        if (err) {
            NSLog(@"[MGJZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
        }
    }
	...
	if ([delegate respondsToSelector:@selector(zipArchiveProgressEvent:total:)]) {
		[delegate zipArchiveProgressEvent:fileSize total:(NSInteger)fileSize];
	}
...
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths {
	BOOL success = NO;
	MGJZipArchive *zipArchive = [[MGJZipArchive alloc] initWithPath:path];
	if ([zipArchive open]) {
...
- (BOOL)close {
	NSAssert((_zip != NULL), @"[MGJZipArchive] Attempting to close an archive which was never opened");
	zipClose(_zip, NULL);
	return YES;
}
...
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime {
    ...
    NSAssert(0xFFFFFFFF == (kYearMask | kMonthMask | kDayMask | kHourMask | kMinuteMask | kSecondMask), @"[MGJZipArchive] MSDOS date masks don't add up");
